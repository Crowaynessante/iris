...
export function showExportChoice(onChoice) {
  ...
  modal.innerHTML = `<div role="dialog" aria-modal="true" style="width:min(460px,100%);background:#fff;border:1px solid #D7E3DA;border-radius:10px;padding:1.5rem;box-shadow:0 20px 60px rgba(15,23,42,.2);"><h3 style="margin:0 0 .4rem;color:#146C36;">Export saved graph</h3><p style="margin:0 0 1rem;color:#64748B;font-size:.85rem;">Choose how to export the selected graph data.</p><div style="display:grid;gap:.55rem;"><button type="button" data-mode="database" class="export-choice-button">Export to Observatory Page <small>(Write to Live MySQL)</small></button><button type="button" data-mode="script" class="export-choice-button">Export as Text File (.txt) <small>(download without database changes)</small></button><button type="button" data-mode="cancel" class="export-cancel-button">Cancel</button></div></div>`;
