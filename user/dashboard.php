...
// Best overall global rank = lowest rank_value (sortable integer)
...
$phRank = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT ph_rank, year FROM rankings WHERE ph_rank IS NOT NULL ORDER BY year DESC LIMIT 1
"));

$bodyCount = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS c FROM ranking_bodies"))['c'];

// Per-body cards (latest year per body)
...
// QS global rank trend (line chart)
...
// Program accreditation (AUN-QA and similar)
...
");
$accreditationRows = [];
while ($row = mysqli_fetch_assoc($accreditations)) {
    $accreditationRows[] = $row;
}
$accreditedCount = count($accreditationRows);
?>
<!DOCTYPE html>
                        ...
                        <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-xl shadow-lg dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
                            <div class="px-4 py-3">
                                ...
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300 mt-1">
                                    <?= is_admin() ? 'ADMINISTRATOR' : 'VIEWER' ?>
                                </span>
                            </div>
                            <ul class="py-2" aria-labelledby="user-menu-button">
                                <?php if (is_admin()): ?>
                                    <li>
                                        <a href="../admin/dashboard.php" class="block px-4 py-2 text-sm text-emerald-600 hover:bg-emerald-50 dark:hover:bg-gray-600 dark:text-emerald-400 font-medium">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-8" id="overview">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gradient-to-r from-emerald-800 to-teal-900 dark:from-emerald-950 dark:to-gray-800 text-white rounded-2xl p-6 shadow-xl relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-wrap items-center gap-3">
                <?php if (is_admin()): ?>
                    <a href="../admin/dashboard.php" class="inline-flex items-center px-4 py-2.5 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl shadow-md transition-all">
